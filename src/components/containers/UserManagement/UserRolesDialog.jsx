...
class createRoles extends PureComponent {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.id && this.props.id !== prevProps.id) {
      this.props.getUserAssociateRole(localStorage.adminToken, this.props.id);
    }
    if (!this.props.roles) {
      this.props.getRoles(localStorage.adminToken);
    }
  ...
  onSaveClick = () => {
    ...
    if (this.props.isRoleMapping) {
      if (!this.state.roles) {
        this.setState({ validationMessage: I18n.t('USER_MANAGEMENT.VALIDATION.PROVIDE_ROLES'), messageType: 'error' });
        return;
      }
      this.props.deleteUserRoleByUserID(localStorage.adminToken, this.props.id, this.props.userAssociateRole);
      ...
      this.props.roleMappingToUser(localStorage.adminToken, this.props.id, newRoleData);
    } else {
      ...
      this.props.createRole(localStorage.adminToken, userEditData);
    }
    this.onHandleCancel();
  };
