...
import { COPY_RIGHT, USER } from "../src/constants/constants";
...
class App extends PureComponent {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.state.user && JSON.stringify(this.state.user) !== JSON.stringify(prevState.user)) {
      ...
      localStorage.setItem('adminToken', state.adminAccess.access_token);
      ...
      const decodeToken= decode(state.adminAccess.access_token);
    }
  }
  render() {
    const { classes } = this.props;
    const copyRightStyle = {
      gridStyle:{height:"3%",backgroundColor:"#295ea7",color:"#fff",width:"100%"},
      pStyle:{float:"right",height:"100%",margin:"0px"}
    }
          ...
          <div style={{ height: "100vh" }}>
            ...
            >
                <KeycloakContext.Provider value={this.state.keycloakData}>
                  ...
                  <Grid style={copyRightStyle.gridStyle}>
                    <p style={copyRightStyle.pStyle}>{COPY_RIGHT}</p>
                  </Grid>
                  </Grid>
